<script lang="ts">
  ...
  async function generate() {
    ...
    if (isPureT2vLongClip()) {
      ...
      if (video.anatomyLock && looksExplicitAnatomy(video.prompt)) {
        logln("anatomy lock: pure T2V long — pos/neg anti feature placement / fused-anatomy guards on the single prompt");
      }
      logln(
  ...
  async function generateSingleSegment() {
    ...
    if (video.anatomyLock && looksExplicitAnatomy(video.prompt) && genPrompt !== video.prompt) {
      logln("anatomy lock: positive anatomy descriptors injected (anti misplaced-feature collapse)");
    }
    try {
  ...
  function continuationPrompt(base: string, longClip = false) {
    ...
    if (video.anatomyLock && explicit) {
      cont +=
        ", consistent anatomy between frames, anatomically correct anatomy";
    }
  ...
  async function generateAutoregressiveLong() {
    ...
    if (video.anatomyLock && looksExplicitAnatomy(originalPrompt)) {
      logln("anatomy lock: positive anatomy descriptors + anti misplaced-feature negatives on every chunk");
    }
    ...
    if (!video.error && video.resultB64) {
      ...
      if (video.anatomyLock && looksExplicitAnatomy(originalPrompt) && video.doRefine) {
        logln("anatomy tip: Refine uses the same anatomy lock language so feature placement mistakes get corrected without re-rolling the whole 30s.");
      }
    }
  }
  ...
  async function enhance() {
    ...
    // the pass actually fixes feature placement and fused-anatomy artifacts.
  ...
  async function generateStoryboard() {
    ...
    if (video.anatomyLock && prompts.some(looksExplicitAnatomy)) {
      logln("anatomy lock: storyboard segments get anatomy-correct descriptors + anti misplaced-feature negatives");
    }
