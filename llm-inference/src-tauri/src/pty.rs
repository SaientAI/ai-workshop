...
//   kairo.bashrc  — sourced via `bash --rcfile`; defines the `kairo` command and
//                   prints a greeting. Re-sources ~/.bashrc so the user's env works.
//   kairo_cli.py  — the Kairo agent TUI. Pure-stdlib Python: renders the ASCII
//                   splash, finds the running tinyq4 server, streams chat over its
//                   OpenAI-compatible SSE endpoint. No external deps, no opencode.

const KAIRO_BASHRC: &str = r##"# Kairo terminal init — generated by AI Workshop. Safe to edit; regenerated on app launch.
unset npm_config_prefix 2>/dev/null
[ -f "$HOME/.bashrc" ] && source "$HOME/.bashrc"

# Launch the Kairo agent TUI (talks to the local tinyq4 server).
kairo() {
  python3 "$HOME/.config/ai-workshop/kairo_cli.py" "$@"
}

# Greeting shown when the terminal opens.
printf '\n\033[38;2;108;142;245mkairo\033[0m \033[2m›\033[0m type \033[1mkairo\033[0m \033[2mto launch the agent\033[0m\n\n'

# To boot straight into the agent every time the terminal opens, uncomment:
# kairo
"##;

const KAIRO_CLI_PY: &str = r####"#!/usr/bin/env python3
# Kairo — local coding agent TUI. Generated by AI Workshop.
...
SYSTEM = """You are Kairo, a local coding agent. You work inside this directory:
__WS__
...
def header(port, model, yolo):
    ...
    if port:
        ...
        print("   %s●%s no tinyq4 server %s· start a model in AI Workshop%s" % (R, X, D, X))
...
def main():
    ...
    while True:
        ...
        if not port:
            port, model = find_server()
            if not port:
                print("   %sno server — load a model in AI Workshop first%s\n" % (R, X)); continue
            header(port, model, yolo)
        messages.append({"role": "user", "content": user})
        # ── Tool loop ──────────────────────────────────────────────────────
        for _ in range(MAX_STEPS):
            print("\n   %skairo%s  " % (A, X), end="", flush=True)
            try:
...
/// Write the Kairo bash init file (and the agent TUI it launches) and return the
/// rcfile path. Returns None if $HOME is unset or the files can't be written
/// (the caller then falls back to a plain shell).
fn write_kairo_rcfile() -> Option<std::path::PathBuf> {
    let home = std::env::var("HOME").ok()?;
    let dir = std::path::PathBuf::from(&home).join(".config/ai-workshop");
    std::fs::create_dir_all(&dir).ok()?;

    let cli = dir.join("kairo_cli.py");
    std::fs::write(&cli, KAIRO_CLI_PY).ok()?;

    let path = dir.join("kairo.bashrc");
    std::fs::write(&path, KAIRO_BASHRC).ok()?;
    Some(path)
}
...
pub async fn pty_spawn(
    ...
    // Launch bash with our Kairo init file so the `kairo` command and banner are
    // available. Fall back to a plain $SHELL if bash or the rcfile is unavailable.
    let rcfile = write_kairo_rcfile();
